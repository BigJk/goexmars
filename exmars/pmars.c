...
/* fake fgets utility so we can read out of a string like a file*/
	...
	if (*redstr != '\0') {
		...
		while (cont) {
			...
			*(mars->buf) = '\0';
			...
			do {
				if (sgets(mars->buf + i, MAXALLCHAR - i, &redstr) != NULL) {
					for (; mars->buf[i]; i++)
						if (mars->buf[i] == '\n' || mars->buf[i] == '\r')
							break;
					mars->buf[i] = 0;
		...
		errprn(mars, FNFERR, (line_st *) NULL, redstr);
...
mars_t* init2(char* w1, char* w2, int coresize, int cycles, int maxprocess, int rounds, int maxwarriorlen, int minsep, int pspacesize) {
	mars_t* mars = 0;
	mars = (mars_t*)malloc(sizeof(mars_t));
	memset(mars, 0, sizeof(mars_t));
	mars->rounds = rounds;
...
mars_t* init1(char* w1, int coresize, int cycles, int maxprocess, int rounds, int maxwarriorlen, int minsep, int pspacesize) {
	mars_t* mars = 0;
	mars = (mars_t*)malloc(sizeof(mars_t));
	memset(mars, 0, sizeof(mars_t));
	mars->rounds = rounds;
...
void Fight2Warriors(char* w1, char* w2, int coresize, int cycles, int maxprocess, int rounds, int maxwarriorlen, int minsep, int pspacesize, int fixpos, int *win1, int *win2, int *equal)
{
	...
	/* setup mars */
	mars_t* mars = init2(w1, w2, coresize, cycles, maxprocess, rounds, maxwarriorlen, minsep, pspacesize);
    if (fixpos != -1) {
...
void Fight1Warrior(char* w1, int coresize, int cycles, int maxprocess, int rounds, int maxwarriorlen, int minsep, int pspacesize, int fixpos, int *win1, int *win2, int *equal)
{
	...
	/* setup mars */
	mars_t* mars = init1(w1, coresize, cycles, maxprocess, rounds, maxwarriorlen, minsep, pspacesize);
    if (fixpos != -1) {
	    mars->fixedPosition = fixpos;
    }
